...
// Add Button component if not imported
const Button = ({ label, customClasses, onClick, ...props }: any) => (
  <button 
    className={`bg-accent-brand text-white px-4 py-2 rounded hover:bg-accent-brand/90 transition-colors ${customClasses}`}
    onClick={onClick}
    {...props}
  >
    {label}
...
const EventsPage: React.FC = () => {
  ...
  const applySearch = () => {
    ...
    const searchProcess = eventsData.filter(e => {
      const searchData = [
        ...
        Array.isArray(e.categories) ? e.categories.join(" ") : e.categories,
        e.creator,
        e.attendees
      ]
      .filter(Boolean)
  ...
  // Helper function to safely handle categories
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-7 sm:grid-cols-2 md:grid-cols-3 max-w-6xl mx-auto">
        {featuredEvent.map((event, index) => {
          const categoriesArray = getCategoriesArray(event.categories);
          ...
          return (
            ...
            >
              ...
              <div className="p-6 flex flex-col gap-3">
                ...
                {/* Categories - Fixed to handle different data types */}
                {categoriesArray.length > 0 && (
                  <div className="flex flex-wrap gap-2 mt-2">
                    {categoriesArray.map((category: string, idx: number) => (
                      <span 
                        key={idx}
                        className="px-2 py-1 bg-zinc-100 text-zinc-600 text-xs rounded-full"
                      >
                        {category}
                      </span>
                    ))}
